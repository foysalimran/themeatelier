	...
	<table id="edd_sl_license_keys" class="edd_sl_table edd-table">
		<thead>
			<tr class="edd_sl_license_row">
				...
				<th class="edd_sl_item"><?php esc_html_e( 'Item', 'themeatelier' ); ?></th>
				<th class="edd_sl_details"><?php esc_html_e( 'License Details', 'themeatelier' ); ?></th>
		...
		<?php if ( $license_keys ) : ?>
			...
			<?php foreach ( $license_keys as $license ) : ?>
				<?php
					...
					$renewal_link = $license->get_renewal_url();
				?>
				<tr class="edd_sl_license_row">
					...
					<td>
						...
						<?php if ( ! empty( $child_licenses ) ) : ?>
							<strong><?php esc_html_e( 'Bundle Licenses', 'themeatelier' ); ?></strong>
									...
									<li class="edd-sl-child">
										<span>
											...
											if ( ! edd_software_licensing()->force_increase() ) {
												...
												?>
												&nbsp;&ndash;&nbsp;<a href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Manage Sites', 'themeatelier' ); ?></a>
												<?php
											}
											?>
										</span>
					...
					<td>
						<span class="edd_sl_status_label"><?php esc_html_e( 'Status:', 'themeatelier' ); ?>&nbsp;</span>
						...
						<div class="edd_sl_item_expiration">
							<span class="edd_sl_expires_label edd_sl_expiries_label"><?php 'expired' === $license->status ? esc_html_e( 'Expired:', 'themeatelier' ) : esc_html_e( 'Expires:', 'themeatelier' ); ?>&nbsp;</span>
							<?php if ( $license->is_lifetime ) : ?>
								<?php _e( 'Never', 'themeatelier' ); ?>
							<?php else: ?>
								<?php echo esc_html( date_i18n( 'F j, Y', $license->expiration ) ); ?>
							<?php endif; ?>
						</div>
						<span class="edd_sl_limit_label"><?php esc_html_e( 'Activations:', 'themeatelier' ); ?>&nbsp;</span>
						<span class="edd_sl_limit_used"><?php echo esc_html( $license->activation_count ); ?></span>
						<span class="edd_sl_limit_sep">&nbsp;/&nbsp;</span>
						<span class="edd_sl_limit_max"><?php echo esc_html( 0 !== $license->activation_limit ? $license->activation_limit : __( 'Unlimited', 'themeatelier' ) ); ?></span>
						<?php if ( ! edd_software_licensing()->force_increase() && ( ! in_array( $license->status, array( 'expired', 'disabled' ), true ) ) ) : ?>
							<br/><a href="<?php echo esc_url( add_query_arg( array( 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'Manage Sites', 'themeatelier' ); ?></a>
						<?php elseif ( 'expired' === $license->status && $renewal_link ) : ?>
							<br/><span class="edd_sl_no_management"><?php esc_html_e( 'Renew to manage sites', 'themeatelier' ); ?></span>
						<?php else : ?>
							<br/><span class="edd_sl_no_management"><?php esc_html_e( 'Unable to manage sites', 'themeatelier' ); ?></span>
						<?php endif; ?>
						<?php if ( edd_sl_license_has_upgrades( $license->ID ) && 'expired' !== $license->status ) : ?>
							<span class="edd_sl_limit_sep">&nbsp;&ndash;&nbsp;</span>
							<a href="<?php echo esc_url( add_query_arg( array( 'view' => 'upgrades', 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'View Upgrades', 'themeatelier' ); ?></a>
						<?php elseif ( edd_sl_license_has_upgrades( $license->ID ) && 'expired' === $license->status ) : ?>
							<span class="edd_sl_limit_sep">&nbsp;&ndash;&nbsp;</span>
							<span class="edd_sl_no_upgrades"><?php esc_html_e( 'Renew to upgrade', 'themeatelier' ); ?></span>
						<?php endif; ?>
						<?php if ( $renewal_link ) : ?>
							<?php if ( 'expired' === $license->status && edd_software_licensing()->can_renew( $license->ID ) ) : ?>
								<span class="edd_sl_key_sep">&nbsp;&ndash;&nbsp;</span>
								<a href="<?php echo esc_url( $renewal_link ); ?>"><?php esc_html_e( 'Renew license', 'themeatelier' ); ?></a>
							<?php elseif ( ! $license->is_lifetime && edd_software_licensing()->can_extend( $license->ID ) ) : ?>
								<span class="edd_sl_key_sep">&nbsp;&ndash;&nbsp;</span>
								<a href="<?php echo esc_url( $renewal_link ); ?>"><?php esc_html_e( 'Extend license', 'themeatelier' ); ?></a>
							<?php endif; ?>
						<?php endif; ?>
						<br/>
						<a class="edd_sl_purchase_number" href="<?php echo esc_url( edd_get_success_page_uri( '?payment_key=' . edd_get_payment_key( $payment_id ) ) ); ?>" title="<?php esc_attr_e( 'View Purchase Record', 'themeatelier' ); ?>"><?php printf( __( 'Purchase #%s', 'themeatelier' ), edd_get_payment_number( $payment_id ) ); ?></a>
		...
		<?php else: ?>
			...
			<tr class="edd_sl_license_row !shadow-none">
				<td class="!p-0" colspan="2"><p class="rounded-md px-10 py-10 shadow-lg text-red-500 font-normal border border-solid border-secondary"><?php esc_html_e( 'You currently have no licenses', 'themeatelier' ); ?></p></td>
			</tr>
		<?php endif; ?>
	</table>
...
<?php else : ?>
	<p class="edd-alert edd-alert-warn">
		<?php esc_html_e( 'You must be logged in to view license keys.', 'themeatelier' ); ?>
	</p>
